  ...
  <div class="card">
    <el-form class="card-form" :model="addItem" :rules="addRules" ref="addItemForm" v-if="!cardData.bid">
      <el-form-item prop="name">
        <el-input v-model.trim="addItem.name" :placeholder="$t('zh_cardname')"></el-input>
      </el-form-item>
      <el-form-item prop="code">
        <el-input v-model.trim="addItem.code" :placeholder="$t('zh_cardcode')"></el-input>
      </el-form-item>
    </el-form>
        ...
        <el-row :gutter="16" style="margin-top: 4px" class="con-con">
          <el-col :span="24">
            <div class="min-card">
              ...
              <p class="min-card-value">
                <span v-for="(item, index) in cardData.taskObjList" :key="index">
                  {{ item.taskName }}
                  <i style="font-style: normal" v-if="item.number != cardData.taskObjList.length">-》</i>
                </span>
              </p>
            </div>
          </el-col>
...
export default {
  ...
  data() {
    return {
      ...
      addRules: {
        name: [
          {required: true, message: this.$t("zh_cardname"), trigger: "blur"},
          {min: 1, max: 20, message: this.$t("zh_checkname1"), trigger: "blur"}
        ],
        code: [
          {validator: this.checkCodeVilidate, trigger: "blur"},
          {min: 1, max: 5, message: this.$t("zh_checkcode1"), trigger: "blur"}
        ]
      }
    };
  },
  ...
  methods: {
    checkCodeVilidate(rule, value, callback) {
      if (!value) {
        ...
        if (!reg.test(value)) {
          ...
          this.checkCode(this.addItem, (flag, msg) => {
            if (flag) {
              callback();
            } else {
